...
//
    ...
    private boolean WritePointFeatures(Object Symbol)
    {
        try
        {
            ...
            if (Symbol instanceof ArcMapSymbols.StructMultilayerMarkerSymbol)
            {
                ArcMapSymbols.StructMultilayerMarkerSymbol objTempStruct = null;
                objTempStruct = (ArcMapSymbols.StructMultilayerMarkerSymbol) Symbol;
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Schraffur", ex.getMessage(), ex.getStackTrace(), "WritePointFeatures");
            return false;
        }
    }
    ...
    private boolean WriteLineFeatures(Object Symbol)
    {
        try
        {
            ...
            if (Symbol instanceof ArcMapSymbols.StructMultilayerLineSymbol)
            {
                ArcMapSymbols.StructMultilayerLineSymbol objTempStruct = null;
                objTempStruct = (ArcMapSymbols.StructMultilayerLineSymbol) Symbol;
                maxLayerIdx = objTempStruct.LayerCount;
            }
            for (layerIdx = 0; layerIdx <= maxLayerIdx - 1; layerIdx++)
            {
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Schraffur", ex.getMessage(), ex.getStackTrace(), "WriteLineFeatures");
            return false;
        }
    }
    ...
    private boolean WritePolygonFeatures(Object Symbol)
    {
        ...
        try
        {
            if (Symbol instanceof ArcMapSymbols.StructSimpleFillSymbol)
            {
                WriteSolidFill((ArcMapSymbols.StructSimpleFillSymbol)Symbol);
            }
            else if (Symbol instanceof ArcMapSymbols.StructMarkerFillSymbol)
            {
                WriteMarkerFill((ArcMapSymbols.StructMarkerFillSymbol)Symbol);
            }
            else if (Symbol instanceof ArcMapSymbols.StructLineFillSymbol)
            {
                ArcMapSymbols.StructLineFillSymbol objSymbol=(ArcMapSymbols.StructLineFillSymbol)Symbol;
                if (objSymbol.Angle > 22.5 && objSymbol.Angle < 67.5)
                {
                    WriteSlopedHatching(Symbol);
                }
                else if (objSymbol.Angle > 67.5 && objSymbol.Angle < 112.5)
                {
                    WritePerpendicularHatching(Symbol);
                }
                else if (objSymbol.Angle > 112.5 && objSymbol.Angle < 157.5)
                {
                    WriteSlopedHatching(Symbol);
                }
                else if (objSymbol.Angle > 157.5 && objSymbol.Angle < 202.5)
                {
                    WritePerpendicularHatching(Symbol);
                }
                else if (objSymbol.Angle > 202.5 && objSymbol.Angle < 247.5)
                {
                    WriteSlopedHatching(Symbol);
                }
                else if (objSymbol.Angle > 247.5 && objSymbol.Angle < 292.5)
                {
                    WritePerpendicularHatching(Symbol);
                }
                else if (objSymbol.Angle > 292.5 && objSymbol.Angle < 337.5)
                {
                    WriteSlopedHatching(Symbol);
                }
                else if (objSymbol.Angle > 337.5 && objSymbol.Angle <= 360.0 || objSymbol.Angle >= 0.0 && objSymbol.Angle < 22.5)
                {
                    WritePerpendicularHatching(Symbol);
                }
            }
            else if (Symbol instanceof ArcMapSymbols.StructDotDensityFillSymbol)
            {
                WriteMarkerFill(Symbol); //Ist z.Zt. sowieso nicht m鰃lich, Dichte mit Punktf黮lungen auszugeben
            }
            else if (Symbol instanceof ArcMapSymbols.StructPictureFillSymbol)
            {
                //TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
            }
            else if (Symbol instanceof ArcMapSymbols.StructGradientFillSymbol)
            {
                //TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
            }
            else if (Symbol instanceof ArcMapSymbols.StructMultilayerFillSymbol)
            {
                ArcMapSymbols.StructMultilayerFillSymbol MFS = null;
                MFS = (ArcMapSymbols.StructMultilayerFillSymbol) Symbol;
                ...
                if (MFS.LayerCount == 1)
                {
                    WritePolygonFeatures(MFS.MultiFillLayers.get(0));
                }
                else if (MFS.LayerCount == 2)
                {
                    for (i = MFS.LayerCount - 1; i >= 0; i--)
                    {
                        WritePolygonFeatures(MFS.MultiFillLayers.get(i)); //hier rekursiver Aufruf
                    }
                }
                else if (MFS.LayerCount > 2)
                {
                    for (i = MFS.LayerCount - 1; i >= 0; i--)
                    {
                        if (iSecure <= 1)
                        {
                            WritePolygonFeatures(MFS.MultiFillLayers.get(i));
                        }
                        iSecure++;
                    }
                }
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Schraffur", ex.getMessage(), ex.getStackTrace(), "WritePolygonFeatures");
            return false;
        }
    }
    ...
    private boolean WriteSolidFill(Object Symbol)
    {
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Fl鋍henf黮lung", ex.getMessage(), ex.getStackTrace(), "WriteSimpleFill");
            return false;
        }
    ...
    private boolean WriteMarkerFill(Object Symbol)
    {
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Punktf黮lung", ex.getMessage(), ex.getStackTrace(), "WriteMarkerFill");
            return false;
        }
    ...
    private boolean WriteSlopedHatching(Object Symbol)
    {
        ...
        try
        {
            ...
            if ( ! objSymbol.Color.equals(""))
            {
                ...
                m_objXMLHandle.SetAttributeValue("fill");
                //Die Schraffurfarbe
                m_objXMLHandle.SetElementText(objSymbol.Color);
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Schraffur", ex.getMessage(), ex.getStackTrace(), "WriteSlopedHatching");
            return false;
        }
    }

    private boolean WritePerpendicularHatching(Object Symbol)
    {
        double dDummy = 0;
        try
        {
        ...
        {
            ErrorMsg("Fehler beim Schreiben der Punktf黮lung", ex.getMessage(), ex.getStackTrace(), "WritePerpendicularHatching");
            return false;
        }
    }
